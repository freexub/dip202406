<?php

use app\models\UserBills;
use yii\helpers\Html;
use yii\helpers\Url;
use yii\grid\ActionColumn;
use yii\grid\GridView;

/** @var yii\web\View $this */
/** @var app\models\UserBillsSearch $searchModel */
/** @var yii\data\ActiveDataProvider $dataProvider */

$this->title = 'Мои счета';
$this->params['breadcrumbs'][] = $this->title;
?>
<?= $this->render('new-bill', [
    'model' => $userBillsModel,
]) ?>
<div class="user-bills-index">

    <h1><?= Html::encode($this->title) ?></h1>

    <p>
        <?= Html::a('Создать', ['create'], [
                'class' => 'btn btn-success',
                'data' => [
                    'bs-toggle' => 'modal',
                    'bs-target' => '#exampleModal'
                ]
        ]) ?>
    </p>

    <div class="row">
        <?php foreach ($dataProvider->models as $model) : ?>
        <div class="col col-12 mb-4">

                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">
                            <a href="delete?id=<?=$model->id?>" class="btn btn-outline-secondary btn-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                                    <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
                                    <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
                                </svg>
                            </a>
                            <a href="view?id=<?=$model->id?>" style="text-decoration: none;">
                                <?=$model->title?>
                            </a>
                            <span class="float-md-end">
                                <?=$model->balans?>
                            </span>
                        </h5>
                        <h6 class="card-subtitle mb-2 text-muted text-small"><?=Yii::$app->formatter->asDate($model->date_create, 'dd.MM.Y');?>
                        </h6>
                    </div>
                </div>
        </div>
        <?php endforeach;?>
    </div>
</div>
